<template>
  <div>
    <select name="" id="" @change="changeCity($event)" v-model="selectedCity">
      ...
      <option :value="city.code" v-for="city in cityList" :key="city.code">
        {{ city.title }}
      </option>
    </select>
    <select name="" id="">
    ...
    <select name="" id="">
      <option :value="dong.dongCode" :key="dong.dongCode"
              v-for="dong in dongList.filter(it => it.cityCode ===selectedCity)">{{ dong.dongTitle }}
      </option>
    </select>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      dongList: [
        ...
        {cityCode: '064', dongCode: '3', dongTitle: '제주3동'},
      ],
      selectedDongList: []
    }
  },
  ...
  methods: {
    changeCity(event) {
      ...
      this.selectedDongList = this.dongList.filter(it => it.cityCode === this.selectedCity)
    }
